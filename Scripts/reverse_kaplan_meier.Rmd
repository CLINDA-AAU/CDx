...
fitPH    <- survfit(with(PH, Surv(start,stop, status)~x))
fitnotPH <- survfit(with(notPH, Surv(start,stop, status)~x))

fitPH.cox        <- coxph(with(PH, Surv(start,stop, status)~x))
fitnotPH.cox     <- coxph(with(notPH, Surv(start,stop, status)~x))
...
plot(fitPH, lty = 1:2, fun = "cloglog", main = "Proportional Hazards",
     xlab = "log(Time)", ylab = "log(-log(Survival)", xlim = c(0.5,15))
text(paste("Test for PH: p =", round(fitPH.cox.zph$table[3], 3)), x = 7, y = -6)
...
plot(fitnotPH, lty = 1:2, fun = "cloglog", main = "not Proportional Hazards",
     xlab = "log(Time)", ylab = "log(-log(Survival)", xlim = c(0.5,15))
text(paste("Test for PH: p =", round(fitnotPH.cox.zph$table[3], 3)), x = 7, y =-6)
```
