...
```{r, include=FALSE}
simon.old <- function(HR, nEvents = NA, concordance = "PPV"){
...
```{r}
simon <- function(HR, conf = NA, concordance = "PPV"){
  ## Function to calculate the positive or negative predictive value
  ## using the semi-parametric method from Simon (2015).
...
simFunc <- function(simDist = "weibull", betaZero = 1, betaCens = 3, betaTrue = 1, anc.ev = 1){
  ...
  simon.est <- simon(cox.res.sum$coefficients[2], cox.res.sum$conf.int[3:4])
...
### Try with ggplot
library(survminer)
a <- ggsurvplot(fitPH, data = PH, title = "Proportional Hazards", legend = "none") + labs(tag = "A")
b <- ggsurvplot(fitPH, data = PH, title = "Proportional Hazards", legend = "none", fun = "cloglog") + labs(tag = "B")
c <- ggsurvplot(fitnotPH, data = notPH, title = "Not Proportional Hazards", legend = "none") + labs(tag = "C")
d <- ggsurvplot(fitnotPH, data = notPH, title = "Not Proportional Hazards", legend = "none", fun = "cloglog") + labs(tag = "D")
arrange_ggsurvplots(list(a,c,b,d), ncol = 2, nrow = 2) 
```
...
latex(resultsTable.tmp,
      ...
      cgroup = c("Simulation", "Simon", "NonPar"),
      n.cgroup = c(2,2,2),
      rowlabel = NA,
...
results[1,1] <- simon(0.45, c(0.34, 0.79))$pub
results[1,2] <- simon(amadoCoxB$coefficients[2], amadoCoxB$conf.int[3:4])$pub

## Simon NPV from published or recalculated IPD data
results[2,1] <- simon(0.99, c(0.73, 1.36), "NPV")$pub
results[2,2] <- simon(amadoCoxA$coefficients[2], amadoCoxA$conf.int[3:4], "NPV")$pub
...
## Simon PPV from published or recalculated IPD data
results[1,1] <- simon(0.48, c(0.36, 0.64))$pub
results[1,2] <- simon(MokCoxB$coefficients[2], MokCoxB$conf.int[3:4])$pub

## Simon NPV from published or recalculated IPD data
results[2,1] <- simon(2.85, c(2.05,3.98), "NPV")$pub
results[2,2] <- simon(MokCoxC$coefficients[2], MokCoxC$conf.int[3:4], "NPV")$pub
